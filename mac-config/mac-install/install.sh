...
# sketchybar-toggle coordinates sketchybar with the native auto-hidden menu bar
# (hover-to-reveal). It is autostarted from sketchybar's init.lua via sbar.exec,
# so it is a hard dependency of the bar behaving correctly. Ensured here rather
# than relying solely on the Brewfile, which is regenerated by `brew bundle dump`.
install_sketchybar_toggle() {
    if brew list malpern/tap/sketchybar-toggle &>/dev/null; then
        print_success "sketchybar-toggle already installed"
        return
    fi

    print_status "Installing sketchybar-toggle for native menu bar coordination..."
