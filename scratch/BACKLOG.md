...
- `mac-config/mac-install/install.sh:98-129` still has its own package-list Stow loop instead of delegating to `scripts/dotfiles stow mac`, so the automated installer can drift from the checked helper command.
