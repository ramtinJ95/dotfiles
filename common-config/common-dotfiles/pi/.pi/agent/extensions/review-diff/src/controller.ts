...
export const CHECKPOINT_ENTRY = "review-diff/checkpoint";
...
export function shouldIgnoreWatchPath(repoRoot: string, path: string, stats?: Stats): boolean {
  const rel = relative(repoRoot, path);
  if (rel === ".git" || rel.startsWith(`.git${sep}`) || rel === "node_modules" || rel.startsWith(`node_modules${sep}`)) {
    return true;
  }
  return stats != null && (stats.isSocket() || stats.isFIFO() || stats.isBlockDevice() || stats.isCharacterDevice());
}
...
export class ReviewController {
  ...
  private async startWatcher(ctx: ExtensionCommandContext): Promise<void> {
    this.watcher = watch(this.repoRoot, {
      ignoreInitial: true,
      ignored: (path, stats) => shouldIgnoreWatchPath(this.repoRoot, path, stats),
    });
    this.watcher.on("error", (error) => {
      const message = error instanceof Error ? error.message : String(error);
      ctx.ui.notify(`Review Loop could not watch part of the repository; automatic refresh may be incomplete: ${message}`, "error");
    });
