...
A local, customizable copy of
[earendil-works/pi-review-loop](https://github.com/earendil-works/pi-review-loop),
vendored from upstream commit `d3549fc`. It is a persistent, incremental diff
reviewer for [pi](https://pi.dev).
...
```sh
cd ~/.pi/agent/extensions/review-diff
npm ci
```

Pi auto-discovers `index.ts` from this directory. This copy has no Git metadata
or automatic upstream update path, so local changes remain under your control.
...
The command returns immediately; it does not replace pi's editor. Running `/review-diff` again brings the existing review window forward.

## Review workflow

1. Open the window with `/review-diff`.
...
Each submitted review appends a `review-diff/checkpoint` custom entry to the active pi session. The checkpoint stores:
...
When the session is resumed, `/review-diff` restores the latest checkpoint on the active session branch and compares it with the current workspace. Session branching therefore also branches review state.
...
The review window closes when pi shuts down, switches sessions, or reloads extensions. Run `/review-diff` again after a reload or session switch.
...
pi -e ~/.pi/agent/extensions/review-diff/index.ts
```
