...
import { shouldIgnoreWatchPath } from "../src/controller.js";
...
test("registers only the local review-diff command", () => {
  ...
  assert.deepEqual(commands, ["review-diff"]);
});

test("ignores Unix sockets when watching a repository", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "review-diff-watch-"));
  ...
  try {
    ...
    assert.equal(shouldIgnoreWatchPath(cwd, socketPath, stats), true);

    const errors: unknown[] = [];
    watcher = watch(cwd, {
      ignoreInitial: true,
      ignored: (path, itemStats) => shouldIgnoreWatchPath(cwd, path, itemStats),
    });
