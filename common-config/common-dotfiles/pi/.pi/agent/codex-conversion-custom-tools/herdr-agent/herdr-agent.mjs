...
export function resolveTargetAlias(target, env = process.env) {
	if (target !== "parent") return target;
	if (!env.SPAWN_AGENT_PARENT_PANE_ID) {
		throw new Error('target "parent" is available only to a spawned subagent');
	}
	return env.SPAWN_AGENT_PARENT_PANE_ID;
}

async function resolvePanel(client, target) {
	target = resolveTargetAlias(target);
	let directError;
	try {
...
export function sessionPath(panel) {
	...
	return {
		...
		actions: {
			...
			send: {
				request: {
					action: "send",
					target: 'pane id, label, or "parent" for a spawned subagent',
					text: "string",
