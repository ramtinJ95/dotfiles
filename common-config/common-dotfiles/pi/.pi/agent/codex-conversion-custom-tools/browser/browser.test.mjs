...
test("host routing mirrors local host and strips transport fields", () => {
	const request = parseRequest('{"host":"server","tabs":[{}]}');
	assert.deepEqual(planHostRoute(request, "igor-server"), {
		host: "server",
		remote: false,
		request: { operations: [{ action: "tabs", offset: 0 }] },
	});
	assert.deepEqual(planHostRoute(request, "desktop"), {
		host: "server",
		remote: true,
		request: { operations: [{ action: "tabs", offset: 0 }] },
	});
...
test("help keeps the preferred loop compact", () => {
	...
	assert.match(help.host, /desktop.*laptop.*server/);
